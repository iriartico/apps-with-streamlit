...
from datetime import (
    datetime,
)  # Importa la clase datetime para trabajar con fechas y horas.
...
def eliminarSurvey(id):
    ...
    st.toast(
        f"Survey {id}: Eliminado exitosamente"
...
)  # Decora la función para que se muestre en un diálogo de Streamlit.
def verFormulario(
    id=None,
):  # Recibe el ID de la encuesta (opcional, para editar una existente).
    ...
    # Continuación del formulario Streamlit.
    with st.form("frmSurvey"):
        ...
        par_years_of_experience = c1.number_input(
            "Años de experiencia", value=val_years_of_experience
        )
        par_country = c2.selectbox(
            "País", listaPaises["name"].unique(), index=val_country
        )
        par_salary = c1.number_input("Salario equivalente en USD", value=val_salary)
        par_main_skills = c2.text_input(
            "Habilidades principales, separadas por coma", value=val_main_skills
        )
        ...
        if btnEnviar:
            itemSurvey = (
                db.SurveyResponse()
            )  # Crea una nueva instancia del objeto SurveyResponse.
            ...
            itemSurvey.country_id = int(
                listaPaises.loc[listaPaises["name"] == par_country, "id"].values[0]
            )
            itemSurvey.role_id = int(
                listaRoles.loc[listaRoles["name"] == par_role, "id"].values[0]
            )
...
with c2:
    ...
    if len(selected.selection.rows) > 0:
        st.button(
            "Editar",
            on_click=verFormulario,
            args=[dfSurveys.loc[selected.selection.rows[0], "id"]],
        )
        st.button(
            "Eliminar",
            on_click=eliminarSurvey,
            args=[dfSurveys.loc[selected.selection.rows[0], "id"]],
        )

        # Descomentar estas líneas para depurar y ver el estado de las selecciones y otros estados internos
